...
2. There should now also be a second ```</activity>``` entry for ```ActivityTwo``` as shown below.
	```XML
	<activity
...
2. Perform the following sequence of applications:
   ...
   * Navigate to ```ActivityTwo```
   * Go back to ```ActivityOne```
   * Rotate the device right into landscape mode
   * Rotate the device left back into portrait mode
   * Rotate the device left again into landscape mode
   * Press the home button to minimize the app (do not change orientation)
   * Re-open the app from the icon deck
8. Record the values displayed in the ```ActivityOne``` screen indicating the following system call totals.
